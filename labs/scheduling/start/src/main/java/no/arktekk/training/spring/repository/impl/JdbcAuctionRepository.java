...
public class JdbcAuctionRepository implements AuctionRepository {
    ...
    public List<Auction> listAllRunningAuctions() {
        ...
        return template.query("select * from Auctions where ? between starts and expires", new AuctionMapper(),
    ...
    public void newAuction(Auction auction) {
        ...
        template.update("insert into Auctions values(?,?,?,?,?)", auction.id(), auction.minimumPrice(),
            timeStampFormatter.print(auction.starts().toDate(), no_NO), timeStampFormatter.print(auction.expires().toDate(), no_NO),
            auction.description());
    }
}
