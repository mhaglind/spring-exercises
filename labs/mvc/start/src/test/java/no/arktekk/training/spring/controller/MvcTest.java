...
public class MvcTest {
	...
	ApplicationContext webappCtx = new FileSystemXmlApplicationContext(
			new String[] { "/src/main/webapp/WEB-INF/auction-servlet.xml" },
			applicationCtx);

	/**
	 * in the auction-servlet.xml add the Index controller as bean either with
	 * xml or with component scanning. If you Component scan, this test will
	...
	@Test
	public void step_4() {
		AuctionController controller = webappCtx
				.getBean(AuctionController.class);
	...
	@Test
	public void step_5() {
		assertNotNull(webappCtx.getBean(RequestMappingHandlerMapping.class));
	}
	...
	public void step_6() throws Exception {
		UrlBasedViewResolver viewResolver = webappCtx
	...
	/**
	 * Configure a message source that looks up properties from
	 * labels.properties
	 * <p/>
	...
	public void step_7() {
		...
		assertEquals("Lagre",
				messageSource.getMessage("form.store", new Object[0], US));
	}
	...
	public void step_8() {
		...
		assertTrue(mappings.keySet().contains("/index.html"));
		assertTrue(mappings.get("/index.html").equals(IndexController.class));
	}
	...
	public void step_9() {
		Map<String, Class<?>> mappings = getHandlerMappings();
		assertTrue(mappings.keySet().contains("/auctions/{auctionId}.html"));
		assertTrue(mappings.get("/auctions/{auctionId}.html").equals(
	 ...
	 * RequestContextListener (to enable session and request attributes) A
	 * servletmapping from the auction servlet to "*.html"
	 * <p/>
	 * Start the webapplication by running the main method in AuctionApp
	 * navigate to http://localhost:8080/ Look around, and inspect how the views
	 * work. Also inspect the jspx files Then when everything is running nice,
	...
	private Map<String, Class<?>> getHandlerMappings() {
		...
		RequestMappingHandlerMapping handlerMapping = webappCtx
